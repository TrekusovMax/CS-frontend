class EncodeDecode {
  ...
  encode(arr: string[]) {
    ...
    encodedStrings.forEach((str) => {
      ...
      str.forEach((byte) => {
        view.setUint32(offset, byte, this.littleEndian)
        offset++
      })
    })

    const atFunc = (index: number) => {
  ...
  decode(buffer: ArrayBuffer) {
    ...
    for (let i = 0; i < size; i++) {
      const strSize = new DataView(bytes.slice(offset, offset + 4).buffer).getUint32(
        0,
        this.littleEndian,
      )
      if (strSize === 0) {
...
export const strings = ['hello', 'мир', ''] // 4 + 4 * 3 + 11 = 27 buffer size
